  ...
  return (
    <div className="space-y-8">
      <div className="bg-white rounded-2xl shadow-lg border border-bs-100 overflow-hidden relative hover:shadow-xl transition-all">
        <div className="relative bg-gradient-to-br from-[#9b87f5] to-[#8B5CF6] p-8">
                ...
                <p className="text-white/80 flex items-center gap-2">
                  ...
                  {filteredWords.length > 0 && (
                    <>
                      ...
                      <span>Sayfa {currentPage}/{Math.ceil(filteredWords.length / itemsPerPage)}</span>
                    </>
                  )}
                </p>
              </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-stretch">
          <div className="flex-1 relative">
            <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
              <Search className="h-5 w-5 text-bs-navygri group-focus-within:text-[#9b87f5] transition-colors" />
            </div>
            <input
              type="text"
              value={searchQuery}
              ...
              className="block w-full pl-12 pr-4 py-3.5 border border-bs-100 rounded-xl text-bs
                       shadow-sm transition-all duration-200 placeholder:text-bs-navygri/50
                       focus:ring-2 focus:ring-[#9b87f5]/10 focus:border-[#9b87f5] focus:shadow-lg
                       hover:border-[#9b87f5]/50 bg-white"
            />
          </div>
          ...
          <div className="w-full sm:w-64">
            <select
              ...
              className="block w-full px-4 py-3.5 border border-bs-100 rounded-xl text-bs-navy
                       ...
                       focus:ring-2 focus:ring-[#9b87f5]/10 focus:border-[#9b87f5] focus:shadow-lg
                       hover:border-[#9b87f5]/50 appearance-none"
            >
        ...
        {filteredWords.length === 0 ? (
          <div className="p-16 text-center">
            <div className="w-24 h-24 rounded-2xl bg-bs-50 flex items-center justify-center mx-auto mb-6
              ...
              <BookOpen className="w-12 h-12 text-[#9b87f5]" />
            </div>
              ...
              {currentWords.map((word, index) => (
                <div
                  ...
                  className="p-6 sm:p-8 hover:bg-gradient-to-br from-bs-50/50 to-white transition-all
                           ...
                           relative overflow-hidden border-l-4 border-l-transparent hover:border-l-[#9b87f5]"
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-[#9b87f5]/5 to-transparent 
                              opacity-0 group-hover:opacity-100 transition-opacity" />

                  <div className="flex-1 relative z-10 min-w-0">
                    <div className="flex flex-wrap items-center gap-3 mb-4">
                      <h3 className="text-2xl sm:text-3xl font-bold text-bs-navy group-hover:text-[#9b87f5]
                  ...
                  <div className="flex flex-wrap items-center gap-4 relative z-10">
                    <span className="text-sm text-bs-navygri font-mono bg-bs-50 px-3 py-2 rounded-lg
                      ...
                      <BookText className="w-4 h-4 text-[#9b87f5]" />
                      {word.pronunciation}
                    </span>
                    <button
                      onClick={() => handleSpeak(word.word)}
                      className="w-10 h-10 flex items-center justify-center text-[#9b87f5]
                             bg-bs-50 hover:bg-white hover:shadow-lg rounded-lg transition-all
                             hover:-translate-y-0.5 border border-transparent hover:border-[#9b87f5]"
                    ...
                    <button
                      ...
                      className="w-10 h-10 flex items-center justify-center text-white
                             bg-[#9b87f5] hover:bg-[#8B5CF6] hover:shadow-lg rounded-lg transition-all
            ...
            {totalPages > 1 && (
              <div className="flex justify-center items-center gap-4 p-6 border-t border-bs-100">
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 text-bs-navygri hover:text-[#9b87f5]
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1)
                    ...
                    .map((page, index, array) => {
                      if (index > 0 && page - array[index - 1] > 1) {
                        return [
                          <span key={`dots-${page}`} className="text-bs-navygri px-2">...</span>,
                          <button
                            key={`page-${page}`}
                            ...
                            className={`w-10 h-10 rounded-lg flex items-center justify-center transition-all
                                    ${currentPage === page
                                      ...
                                      : 'text-bs-navygri hover:bg-bs-50 hover:text-[#9b87f5]'}`}
                          >
                            {page}
                          </button>
                        ];
