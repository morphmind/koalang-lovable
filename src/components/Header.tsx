  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.ctrlKey || e.metaKey) && e.key === 'k') {
        e.preventDefault();
        // Eğer dom'da arama girişi varsa odakla
        const searchInput = document.querySelector('input[placeholder="Kelime veya anlam ara..."]') as HTMLInputElement;
        if (searchInput) {
          searchInput.focus();
        }
      }
    ...
    >
      ...
      <div className="relative bg-gradient-to-r from-bs-navy via-bs-primary to-bs-800 rounded-b-3xl">
        ...
        <div className="relative backdrop-blur-sm border-b border-white/10 rounded-b-3xl shadow-lg">
          <div className={`max-w-7xl mx-auto px-2 ${isMobile ? 'sm:px-4' : 'px-4 sm:px-6 lg:px-8'}`}>
            <div className={`flex items-center gap-4 transition-all duration-300
              ...
              {isSticky && isSearchVisible && (
                <div className={`flex-1 flex items-center justify-between ${isMobile && searchQuery ? 'gap-0.5' : 'gap-1'} sm:gap-4 max-w-4xl mx-auto animate-fade-in-up`}>
                  {/* Modern Arama Kutusu */}
                  <div className={`relative ${isMobile ? searchQuery ? 'max-w-[95%] flex-1' : 'max-w-[80%] flex-1' : 'flex-1 max-w-2xl mx-auto'} group transition-all duration-300`}>
                    <div className={`absolute inset-y-0 left-0 ${isMobile && searchQuery ? 'pl-2' : 'pl-4'} flex items-center pointer-events-none 
                                  transition-all duration-300 group-focus-within:text-indigo-300`}>
                      <Search className={`${isMobile && searchQuery ? 'h-4 w-4' : 'h-5 w-5'} text-white/60 group-focus-within:text-white`} />
                    </div>
                    <input
                      type="text"
                      value={searchQuery}
                      ...
                      className={`w-full ${isMobile ? searchQuery ? 'pl-8 pr-8 py-1.5' : 'pl-10 pr-10 py-1.5' : 'pl-12 pr-12 py-2'} bg-white/10 border border-white/20 rounded-lg
                                 text-white placeholder-white/60 
                                 focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 focus:bg-white/15
                                 transition-all duration-300 ${isMobile ? 'text-xs' : 'text-sm'}
                                 hover:bg-white/15 hover:border-white/30`}
                    />
                    {searchQuery && (
                      <button 
                        onClick={() => setSearchQuery('')}
                        className={`absolute inset-y-0 right-0 ${isMobile ? searchQuery ? 'pr-1' : 'pr-2' : 'pr-4'} flex items-center text-white/60 hover:text-white
                                  transition-all duration-300`}
                      >
                        <X className={`${isMobile ? 'h-3 w-3' : 'h-4 w-4'}`} />
                      </button>
                    )}
                    <div className="absolute right-4 top-0 h-full pointer-events-none flex items-center">
                      <div className={`transition-all duration-300 overflow-hidden ${searchQuery ? 'w-0 opacity-0' : 'w-auto opacity-100'} ${isMobile ? 'hidden' : ''}`}>
                        <div className="bg-white/10 text-white/60 text-xs px-2 py-1 rounded">
                          Ctrl + K
                        </div>
                      </div>
                    </div>
                  ...
                  <button
                    ...
                    className={`flex items-center justify-center ${isMobile ? searchQuery ? 'w-7 min-w-0' : 'w-8 min-w-0' : 'w-10 sm:w-auto'} px-2 sm:px-4 py-2
                               rounded-lg ${isMobile ? 'text-xs' : 'text-sm'} font-medium transition-all ${
                                 showLearned
                  ...
                  >
                    {showLearned ? (
                      <BookmarkCheck className={`${isMobile ? 'w-4 h-4' : 'w-5 h-5'}`} />
                    ) : (
                      <BookmarkPlus className={`${isMobile ? 'w-4 h-4' : 'w-5 h-5'}`} />
                    )}
                  ...
                  <button
                    ...
                    className={`flex items-center justify-center ${isMobile ? searchQuery ? 'w-7 min-w-0' : 'w-8 min-w-0' : 'w-10 sm:w-auto'} px-2 sm:px-4 py-2
                               bg-white text-bs-primary rounded-lg ${isMobile ? 'text-xs' : 'text-sm'} font-medium hover:bg-bs-50
                               transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5`}
                  >
                    <PenTool className={`${isMobile ? 'w-4 h-4' : 'w-5 h-5'}`} />
              ...
              <div className="flex items-center gap-2 pr-4 lg:pr-6 ml-auto">
                {user ? (
                  <>
                    {/* Mobilde sticky modda gizle, desktop'ta her zaman göster */}
                    {!(isSticky && isMobile) && <NotificationBell />}
                    <UserMenu />
                  </>
                ) : (
                ...
                <button
                  ...
                  className="md:hidden w-8 h-8 flex items-center justify-center rounded-lg 
                         hover:bg-white/10 transition-colors ml-1"
